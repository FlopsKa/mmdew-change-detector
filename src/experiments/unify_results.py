...
def load_data(folder):
    ...
    for col in ["actual_cps", "detected_cps_at"]:
...
if __name__=="__main__":
    ...
    avg_results = df[["dataset", "algorithm", "config", "runtime", "percent_changes_detected", "f1_detected_cps_at", "precision", "recall"]].groupby(["dataset", "algorithm", "config"]).mean().reset_index().fillna(0)
